...
import { ElectronicsInfo, ElectronicsInfoService } from '../electronics-info';
...
export class OfferDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.electronicsInfoService
            .query({filter: 'offer-is-null'})
            .subscribe((res: HttpResponse<ElectronicsInfo[]>) => {
                if (!this.offer.electronicsInfo || !this.offer.electronicsInfo.id) {
                    this.electronicsinfos = res.body;
                } else {
                    this.electronicsInfoService
                        .find(this.offer.electronicsInfo.id)
                        .subscribe((subRes: HttpResponse<ElectronicsInfo>) => {
                            this.electronicsinfos = [subRes.body].concat(res.body);
                        }, (subRes: HttpErrorResponse) => this.onError(subRes.message));
                }
            }, (res: HttpErrorResponse) => this.onError(res.message));
    ...
    trackElectronicsInfoById(index: number, item: ElectronicsInfo) {
        return item.id;
    }

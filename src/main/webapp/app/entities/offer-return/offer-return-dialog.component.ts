...
import { OfferPayment, OfferPaymentService } from '../offer-payment';
...
export class OfferReturnDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.offerPaymentService
            .query({filter: 'offerreturn-is-null'})
            .subscribe((res: HttpResponse<OfferPayment[]>) => {
                if (!this.offerReturn.payment || !this.offerReturn.payment.id) {
                    this.payments = res.body;
                } else {
                    this.offerPaymentService
                        .find(this.offerReturn.payment.id)
                        .subscribe((subRes: HttpResponse<OfferPayment>) => {
    ...
    trackOfferPaymentById(index: number, item: OfferPayment) {
        return item.id;
    }

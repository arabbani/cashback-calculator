import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpResponse, HttpErrorResponse } from '@angular/common/http';
import { Subscription } from 'rxjs/Subscription';
import { JhiEventManager, JhiAlertService } from 'ng-jhipster';

import { FlightInfo } from './flight-info.model';
import { FlightInfoService } from './flight-info.service';
import { Principal } from '../../shared';

@Component({
    selector: 'apsstr-flight-info',
    templateUrl: './flight-info.component.html'
})
export class FlightInfoComponent implements OnInit, OnDestroy {
flightInfos: FlightInfo[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        private flightInfoService: FlightInfoService,
        private jhiAlertService: JhiAlertService,
        private eventManager: JhiEventManager,
        private principal: Principal
    ) {
    }

    loadAll() {
        this.flightInfoService.query().subscribe(
            (res: HttpResponse<FlightInfo[]>) => {
                this.flightInfos = res.body;
            },
            (res: HttpErrorResponse) => this.onError(res.message)
        );
    }
    ngOnInit() {
        this.loadAll();
        this.principal.identity().then((account) => {
            this.currentAccount = account;
        });
        this.registerChangeInFlightInfos();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: FlightInfo) {
        return item.id;
    }
    registerChangeInFlightInfos() {
        this.eventSubscriber = this.eventManager.subscribe('flightInfoListModification', (response) => this.loadAll());
    }

    private onError(error) {
        this.jhiAlertService.error(error.message, null, null);
    }
}
